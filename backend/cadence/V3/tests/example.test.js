...
describe("VotingCommunity Contract", () => {
    ...
    test("Requesting Admin Proxy", async () => {
        ...
        const GrantResourceDistributorCapability = (await getTransactionCode({
            name: "GrantResourceDistributorCapability",
            addressMap
        })).toString()
        ...
        for (let acct of UserAccounts) {
            ...
            const privatePath = `/private/VotingCommunityResourceDistributor_${acct}`
            let GrantResourceDistributorCapabilityCode = GrantResourceDistributorCapability 
              .replace(/%PRIVATE_PATH%/ig, privatePath)
            
              console.log(GrantResourceDistributorCapabilityCode)

            const txResult = await shallPass(
                sendTransaction({ code: GrantResourceDistributorCapabilityCode, args: [acct], signers })
            )
        ...
        ///////////////////////////////////////////////////////////////////
        // Create & Store SuperAdmin via AdminProxy:
        ///////////////////////////////////////////////////////////////////

        const CreateAndStoreSuperAdminCode = await getTransactionCode({
          name: "CreateAndStoreSuperAdmin",
          addressMap
        })
        console.log(CreateAndStoreSuperAdminCode)

        // Accounts w/ ResourceDistributor Capability should be able to create & store SuperAdmin resource in their own storage
        for (let acct of UserAccounts) {
          const signers = [acct]
          const args = []
          const txResult = await shallPass(
              sendTransaction({ code: CreateAndStoreSuperAdminCode, args, signers })
          )
          expect(txResult[0].status).toBe(SUCCESS_CODE)
        }

        ///////////////////////////////////////////////////////////////////
        // Create & Store CommunityCollection via SuperAdmin:
        ///////////////////////////////////////////////////////////////////

        const CreateAndStoreCollectionAsSuperAdmin = await getTransactionCode({
          name: "CreateAndStoreCollectionAsSuperAdmin",
          addressMap
        })
        console.log(CreateAndStoreCollectionAsSuperAdmin)

        // SuperAdmins should be able to save CommunityCollection resources to their own storage
        for (let acct of UserAccounts) {
          const signers = [acct]
          const args = []
          const txResult = await shallPass(
              sendTransaction({ code: CreateAndStoreCollectionAsSuperAdmin, args, signers })
          )
          expect(txResult[0].status).toBe(SUCCESS_CODE)
        }
        ...
        ///////////////////////////////////////////////////////////////////
        // SuperAdmin Create & Store Community via CommunityCollection:
        ///////////////////////////////////////////////////////////////////

        const CreateAndStoreCommunityAsSuperAdmin = await getTransactionCode({
          name: "CreateAndStoreCommunityAsSuperAdmin",
          addressMap
        })
        console.log(CreateAndStoreCommunityAsSuperAdmin)

        // SuperAdmins should be able to save Communitiy resources to CommunityCollection in their own storage
        for (let acct of UserAccounts) {
          const signers = [acct]
          const args = [`Flow Community by ${acct}`, `Cool Flow Community created by address: ${acct}`, {}]
          const txResult = await shallPass(
              sendTransaction({ code: CreateAndStoreCommunityAsSuperAdmin, args, signers })
          )
          expect(txResult[0].status).toBe(SUCCESS_CODE)
        }
      });
