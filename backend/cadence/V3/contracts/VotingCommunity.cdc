...
pub contract VotingCommunity {
    pub let RESOURCE_DISTRIBUTOR_PATH: StoragePath
    ...
    pub resource interface AdminReceiver {
        ...
        pub fun setResourceDistributorCapability(capability: Capability<&ResourceDistributor>)
    }

    pub resource AdminProxy: AdminReceiver, Admin {
        access(self) var adminCapability: Capability<&CommunityCollection>?
        access(self) var resourceDistributorCapability: Capability<&ResourceDistributor>?

        init() {
            self.adminCapability = nil
            self.resourceDistributorCapability = nil
        ...
        pub fun setResourceDistributorCapability(capability: Capability<&ResourceDistributor>) {
            self.resourceDistributorCapability = capability
        }
        ...
        // Allows the owner of the AdminProxy resource to createAndStore community
        // in the SuperAdmin's CommunityCollection resource, IF they have the linked capability
        pub fun createAndStoreCommunity(name: String, description: String, meta: {String: String}) {
        ...
        pub fun createSuperAdmin(): @SuperAdmin {
            return <- self.resourceDistributorCapability!
                .borrow()!
                .createSuperAdmin()
        }
    ...
    pub resource SuperAdmin {
        ...
        pub fun trackCapability(adminAddress: Address, privatePath: PrivatePath) {
            self.distributedCapabilities[adminAddress] = privatePath
            emit AdminCapabilityDistributed(to: adminAddress, meta: nil)
        }
    }

    // Resource that allows caller to create SuperAdmin resource for their
    // own storage.  Deployer account must link a capability to the caller account's
    // admin proxy resource before they can interact with this.
    pub resource ResourceDistributor {
        pub let distributedCapabilities: {Address: Path}
        init() {
            self.distributedCapabilities = {}
        }
    ...
    init() {
        self.RESOURCE_DISTRIBUTOR_PATH = /storage/DAPPER_COLLECTIVE_VOTING_COMMUNITY_Resource_Distributor
        ...
        self.account.save<@ResourceDistributor>(<- create ResourceDistributor(), to: self.RESOURCE_DISTRIBUTOR_PATH)
