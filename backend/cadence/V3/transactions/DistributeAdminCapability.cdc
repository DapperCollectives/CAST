...
transaction(adminRequesterAddress: Address) {
    ...
    prepare(superAdmin: AuthAccount) {
        ...
        // link capability
        self.adminCapability = superAdmin.link<&VotingCommunity.CommunityCollection>(self.capabilityPrivatePath, target: VotingCommunity.COMMUNITY_COLLECTION_PATH) ??
            panic("Could not link admin capability")
        
        // track the capability in-contract
        superAdminCap.trackCapability(adminAddress: self.adminRequester.address, storagePath: VotingCommunity.COMMUNITY_COLLECTION_PATH)
